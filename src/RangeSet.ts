...
export class RangeSet {
  ...
  public shiftDelta(delta: textUtil.RangeDelta) {
    this.ranges.forEach((r,idx) => {
      this.ranges[idx] = textUtil.rangeTranslate(this.ranges[idx], delta);
    })
  }

  private getOverlappingIndices(range: vscode.Range, options: {includeTouchingStart:boolean,includeTouchingEnd:boolean}) : {begin: number, end: number} {
  ...
  public getOverlapping(range: vscode.Range, options: {includeTouchingStart:boolean,includeTouchingEnd:boolean}) : vscode.Range[] {
  ...
  public removeOverlapping(range: vscode.Range, options: {includeTouchingStart:boolean,includeTouchingEnd:boolean}) : vscode.Range[] {
    const {begin: begin, end: end} = this.getOverlappingIndices(range, options);
